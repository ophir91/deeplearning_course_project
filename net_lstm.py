...
class LSTMClassifaierAndDenoise(nn.Module):
    def __init__(self, M):
        ...
        self.oneDenoise = LSTMDenoise(input_dim=257, hidden_dim=512, output_size=257, num_of_deep_lstm=3)
        self.MDenoise = nn.ModuleList()
        for i in range(M):
            self.MDenoise.append(self.oneDenoise)

        self.classifier = LSTMblock(input_dim=39, hidden_dim=512, output_size=M, num_of_deep_lstm=3)

    def forward(self, stft, mfcc):
        N = mfcc.shape[0]  # number of frames
        # maps = torch.zeros(N, 257).cuda()
        ...
        for i, one_denoise in enumerate(self.MDenoise):
            ...
            d[i, :, :] = one_denoise_output * c[:,i:i+1]
            # maps += (one_denoise_output * c[:,i:i+1])
        maps = torch.sum(d, dim=0)
        #
        # for j in range(N):
...
class LSTMblock(nn.Module):
    ...
    def __init__(self, input_dim=257, hidden_dim=512, output_size=257, num_of_deep_lstm=1):
        ...
        self.num_of_deep_lstm = num_of_deep_lstm
    ...
    def init_hidden(self,num_of_deep_lstm=1):
        # Before we've done anything, we dont have any hidden state.
        # Refer to the Pytorch documentation to see exactly
        # why they have this dimensionality.
        # The axes semantics are (num_layers, minibatch_size, hidden_dim)
        # return torch.zeros(1, 1, self.hidden_dim)
        return (torch.zeros(num_of_deep_lstm, 1, self.hidden_dim).cuda(),
                torch.zeros(num_of_deep_lstm, 1, self.hidden_dim).cuda())

    def forward(self, stft):
        # self.hidden = self.init_hidden(self.num_of_deep_lstm)
...
class LSTMDenoise(nn.Module):
    ...
    def forward(self, stft):
        ...
        lstm_out, self.hidden = self.lstm(
            stft.view(len(stft), 1, -1))#, self.hidden)
        out_space = self.hidden2out(lstm_out.view(len(stft), -1))
        out_scores = F.sigmoid(out_space)
        return out_scores
